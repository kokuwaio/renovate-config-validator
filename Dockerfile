...
RUN --mount=type=cache,target=/build \
	...
	wget --no-hsts --quiet \
		"https://nodejs.org/download/release/v22.21.1/node-v22.21.1-linux-$ARCH.tar.xz" \
		"https://nodejs.org/download/release/v22.21.1/SHASUMS256.txt" \
		"https://nodejs.org/download/release/v22.21.1/SHASUMS256.txt.sig" && \
	sha256sum --quiet --check --strict --ignore-missing SHASUMS256.txt && \
	gpg --verify SHASUMS256.txt.sig SHASUMS256.txt && \
	tar --xz --extract --file="node-v22.21.1-linux-$ARCH.tar.xz" --exclude=bin/npx --exclude=bin/corepack --exclude=lib/node_modules/corepack --exclude=include --exclude=share --no-same-owner && \
	mv "node-v22.21.1-linux-$ARCH" /opt/node && \
...
RUN --mount=type=cache,target=/tmp PATH="$PATH:/opt/node/bin" npm install "renovate@42.66.8" --global --no-fund --cache=/tmp && \
...
FROM docker.io/library/debian:13.2-slim@sha256:def4126bd4619cd54d179f12ed9eb5438cd7b897e9cf523b5a3d3488274ed8aa
