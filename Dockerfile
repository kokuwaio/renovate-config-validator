...
RUN --mount=type=cache,target=/build,id=nodejs \
	...
	wget --no-hsts --quiet \
		"https://nodejs.org/download/release/v24.20.0/node-v24.20.0-linux-$ARCH.tar.xz" \
		"https://nodejs.org/download/release/v24.20.0/SHASUMS256.txt" \
		"https://nodejs.org/download/release/v24.20.0/SHASUMS256.txt.sig" && \
	sha256sum --quiet --check --strict --ignore-missing SHASUMS256.txt && \
	gpg --verify SHASUMS256.txt.sig SHASUMS256.txt && \
	tar --xz --extract --file="node-v24.20.0-linux-$ARCH.tar.xz" --exclude=bin/npx --exclude=bin/corepack --exclude=lib/node_modules/corepack --exclude=include --exclude=share --no-same-owner && \
	mv "node-v24.20.0-linux-$ARCH" /opt/node && \
...
RUN --mount=type=cache,target=/tmp PATH="$PATH:/opt/node/bin" npm install "renovate@44.69.12" --global --no-fund --cache=/tmp && \
...
FROM docker.io/library/debian:13.6-slim@sha256:d7e12182ce18b85b93007c1dedf31f2d29e01ccf3182cc4017c709b6259bc132 AS renovate-config-validator
