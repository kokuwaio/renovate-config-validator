...
FROM docker.io/library/debian:13.5-slim@sha256:f3da28155e2e26086464eba22cd235b22200b7143e8f3e1811bf359e3114bf96 AS build
...
RUN --mount=type=cache,target=/build,id=nodejs \
	...
	wget --no-hsts --quiet \
		"https://nodejs.org/download/release/v24.18.0/node-v24.18.0-linux-$ARCH.tar.xz" \
		"https://nodejs.org/download/release/v24.18.0/SHASUMS256.txt" \
		"https://nodejs.org/download/release/v24.18.0/SHASUMS256.txt.sig" && \
	sha256sum --quiet --check --strict --ignore-missing SHASUMS256.txt && \
	gpg --verify SHASUMS256.txt.sig SHASUMS256.txt && \
	tar --xz --extract --file="node-v24.18.0-linux-$ARCH.tar.xz" --exclude=bin/npx --exclude=bin/corepack --exclude=lib/node_modules/corepack --exclude=include --exclude=share --no-same-owner && \
	mv "node-v24.18.0-linux-$ARCH" /opt/node && \
...
RUN --mount=type=cache,target=/tmp PATH="$PATH:/opt/node/bin" npm install "renovate@43.241.5" --global --no-fund --cache=/tmp && \
	find /opt/node/lib/node_modules/renovate -type f ! -name \*.js -a ! -name \*.cjs -a ! -name \*.mjs -a ! -name \*.wasm.gz -a ! -name \*.wasm -a ! -name package.json -a ! -name vendors.json -a ! -name mappingTable.json -a ! -name builtin-modules.json -a ! -path '/opt/node/lib/node_modules/renovate/dist/data/*' -delete && \
...
FROM docker.io/library/debian:13.5-slim@sha256:f3da28155e2e26086464eba22cd235b22200b7143e8f3e1811bf359e3114bf96 AS renovate-config-validator
